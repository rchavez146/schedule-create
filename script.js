...
function syncBranchHoursControls() {
  ...
  if (branchHoursStatus) {
    const closedLabel = closedDays.size ? ` • Closed: ${[...closedDays].map((day) => day.slice(0, 3)).join(", ")}` : "";
    branchHoursStatus.textContent = `Weekday ${hours.weekdayOpen}-${hours.weekdayClose} • Saturday ${hours.saturdayOpen}-${hours.saturdayClose}${closedLabel}`;
  }
}
...
function renderCellStationLine(stationRoot, assignment) {
  ...
  if (!station || assignment.shift === darkCellShift) {
...
function normalizeLoadedSchedule(raw) {
  ...
  return {
    ...
    payload: {
      ...
      branchClosedDays: normalizeBranchClosedDays(payload.branchClosedDays),
...
function normalizeBranchClosedDays(days = []) {
...
function applyClosedDayRules() {
  const closedDays = normalizeBranchClosedDays(state.branchClosedDays);
