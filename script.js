...
function createSaturdayRuleForOffWeeks(offWeeks, label = "") {
  ...
  return createSaturdayRule(
    weekTypeOptions.filter((weekType) => !offSet.has(weekType)),
    label
  );
}
...
const openFirstTimeSetupButton = document.querySelector("#openFirstTimeSetup");
...
const branchRulesStationSetupButton = document.querySelector("#branchRulesStationSetup");
...
const branchRulesSaveButton = document.querySelector("#branchRulesSave");
...
function bindEvents() {
  ...
  weekTypeInput.addEventListener("change", async (event) => {
    const accepted = await askForConfirmation({
      ...
      message: "This will rebuild the week template. Week A through Week H use the employee Saturday rotation rules, while Custom leaves Saturday open for manual planning.",
  ...
  async function createBranchTemplateExport() {
    ...
    const accepted = await askForConfirmation({
      ...
      message: "This exports employees, roles, target hours, fixed days off, all A-H Saturday rotation rules, branch shift times, auto-fill options, station settings, and custom task definitions. Weekly notes, helper rows, PTO entries, colors, day events, and task assignments will be left out.",
  ...
  openFirstTimeSetupButton?.addEventListener("click", () => {
  ...
  branchRulesStationSetupButton?.addEventListener("click", () => {
  ...
  branchRulesSaveButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = false;
    syncSetupReturnButtons();
    saveBranchRules();
  });
...
function syncSetupReturnButtons() {
  [backToSetupFromEmployeesButton, backToSetupFromBranchRulesButton, backToSetupFromStationButton].forEach((button) => {
    button?.classList.toggle("is-hidden", !firstTimeSetupFlowActive);
  });
}
...
function buildBranchShiftRow(shift) {
  ...
  removeButton.addEventListener("click", () => {
    ...
    branchShiftStatus.textContent = "Shift time removed. Click Save Branch Rules to keep this change.";
  });
...
function addBranchShiftFromInput() {
  ...
  branchShiftInput.value = "";
  branchShiftStatus.textContent = `${normalized} added. Click Save Branch Rules to keep this change.`;
}
...
function syncStationAutoFillModal() {
  ...
  stationAutoFillLobbyMinutesInput.value = String(settings.lobbyLeadMinutes);
  stationAutoFillRandomizeInput.checked = settings.randomize;
  stationAutoFillStatus.textContent = settings.locations.length
  ...
  renderStationLocationRows(settings.locations);
}

function getStationAutoFillSettingsFromInputs() {
  return normalizeStationAutoFillSettings({
    locations: getStationLocationsFromModal(),
    randomize: stationAutoFillRandomizeInput.checked,
    lobbyLeadMinutes: stationAutoFillLobbyMinutesInput.value,
  });
}

function renderStationLocationRows(locations = []) {
  stationAutoFillLocations.innerHTML = "";
  if (locations.length === 0) {
    stationAutoFillLocations.appendChild(buildEmptyStateRow("No stations/offices yet. Add the locations this branch uses."));
    return;
  }

  locations.forEach((location) => {
    stationAutoFillLocations.appendChild(buildStationLocationRow(location));
  });
}

function buildStationLocationRow(location) {
  const row = document.createElement("div");
  ...
  removeButton.addEventListener("click", () => {
    row.remove();
    if (stationAutoFillLocations.querySelectorAll(".station-location-row").length === 0) {
      stationAutoFillLocations.appendChild(buildEmptyStateRow("No stations/offices yet. Add the locations this branch uses."));
    }
    updateStationSetupFeedback(`${location.label} removed.`);
  });
...
function addStationLocation(type) {
  const current = getStationLocationsFromModal();
  ...
  stationAutoFillLocations.querySelector(".setup-empty-state")?.remove();
  stationAutoFillLocations.appendChild(buildStationLocationRow(location));
  updateStationSetupFeedback(`${label} added. Add a lunch time if this location has a set lunch.`);
}

function getStationLocationsFromModal() {
  return Array.from(stationAutoFillLocations.querySelectorAll(".station-location-row"))
    .map((row) => ({
      id: row.dataset.id,
...
function saveStationSetupRules() {
  state.stationAutoFillSettings = getStationAutoFillSettingsFromInputs();
  setDirtyAndAutosave("Station setup saved.");
  renderSaveStatus();
  renderWorkspaceTabs();
  stationAutoFillStatus.textContent = state.stationAutoFillSettings.locations.length
    ? `${state.stationAutoFillSettings.locations.length} stations/offices saved.`
    : "Station setup saved with no stations/offices.";
  renderStationLocationRows(state.stationAutoFillSettings.locations);
  showToast("Station setup saved.");
}

function updateStationSetupFeedback(message) {
  const count = getStationLocationsFromModal().length;
  stationAutoFillStatus.textContent = `${message} ${count} total stations/offices.`;
}
...
function saveEmployee() {
  ...
  const saturdayGroup = isAssistRow ? "1" : employeeSaturdayGroupInput.value;
...
function getDefaultSaturdayRuleForEmployee(employee) {
  if (employee.saturdayRule?.workWeeks?.length) {
    return {
      workWeeks: employee.saturdayRule.workWeeks.filter((week) => weekTypeOptions.includes(week)),
  ...
  if (rawSaturdayGroup === "odd") {
    return createSaturdayRuleForOffWeeks(["Week A", "Week C", "Week E", "Week G"], "A/C/E/G Off");
  }
  if (rawSaturdayGroup === "even") {
    return createSaturdayRuleForOffWeeks(["Week B", "Week D", "Week F", "Week H"], "B/D/F/H Off");
  }

  const saturdayGroup = [0, 1, 2, 3, 4, 5, 6, 7, 8].includes(Number(rawSaturdayGroup))
    ? Number(rawSaturdayGroup)
    : 1;
  if (saturdayGroup === 0) {
    return createSaturdayRule(weekTypeOptions, "Works every rotating Saturday");
  }
...
function normalizeLoadedSchedule(raw) {
  ...
  const normalizedWeekType = [...weekTypeOptions, "Custom"].includes(payload.currentWeekType)
...
function hydrateEmployees(employeeList) {
  employeeList.forEach((employee, index) => {
    ...
    employee.saturdayGroup =
      ["odd", "even"].includes(saturdayGroup) || [0, 1, 2, 3, 4, 5, 6, 7, 8].includes(Number(saturdayGroup))
        ? saturdayGroup
        : "1";
