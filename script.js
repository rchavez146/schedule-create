...
const scheduleDataVersion = 11;

const weekTypeOptions = ["Week A", "Week B", "Week C", "Week D", "Week E", "Week F", "Week G", "Week H"];
const defaultRotationWeekCount = "";
const fallbackRotationWeekCount = 3;
...
function createSaturdayRuleForOffGroup(offGroup, label = "") {
  ...
  return createSaturdayRuleForOffWeeks([offWeek], label);
}

function createSaturdayRuleForOffWeeks(offWeeks, label = "") {
  const offSet = new Set(offWeeks);
  return createSaturdayRule(
    getActiveWeekTypeOptions().filter((weekType) => !offSet.has(weekType)),
    label
  );
}
...
// Demo branches start with no location-specific shift rules. Each branch adds
// only the times it actually uses, then saves those rules into its template.
const defaultBranchShiftPresets = [];
// If a user adds employees before creating Branch Rules, use these hidden
// starter shifts so the new row is workable instead of all OFF. They do not
// appear as saved branch rules unless the user adds them.
const starterScheduleShiftPresets = [
  "8:00-6:00",
  "8:30-6:00",
  "8:00-5:30",
  "8:30-5:30",
...
// Sunday is displayed as a branch-closed day for manual event scheduling only.
// Auto-fill routines use autoFillWeekDays so a Sunday event never gets overwritten.
const weekDays = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"];
const autoFillWeekDays = weekDays.filter((day) => day !== "Sunday");
...
const cellColorOptions = [
  ...
  { value: "#e7ddff", label: "Light Purple" },
  { value: "#ff5cad", label: "Hot Pink" },
  { value: "#a8f1ed", label: "Teal" },
];
...
const state = {
  currentWeekType: "Custom",
  rotationWeekCount: defaultRotationWeekCount,
  rotationWeekCountConfigured: false,
  ...
  // Sundays are normally closed, but Sunday cells remain available for manual
  // events and those manually entered hours still count toward totals.
  branchClosedDays: ["Sunday"],
  stationAutoFillSettings: {
    stations: 0,
    offices: 0,
    locations: [],
    randomize: true,
  ...
  customTasks: [],
  branchShiftPresets: [],
  branchPartTimeShiftPresets: [],
  customShiftPresets: [],
  cellEditorDock: "right",
  cellEditorExpanded: false,
  loadedSaveId: "",
...
const openBranchRulesButton = document.querySelector("#openBranchRules");
...
const backToSetupFromEmployeesButton = document.querySelector("#backToSetupFromEmployees");
const backToBranchRulesFromEmployeesButton = document.querySelector("#backToBranchRulesFromEmployees");
...
const employeeTaskExemptionOptions = Array.from(document.querySelectorAll(".employee-task-exemption"));
...
const employeeShiftPoolTypeInput = document.querySelector("#employeeShiftPoolType");
const employeeTargetHoursInput = document.querySelector("#employeeTargetHours");
const employeeStationExemptInput = document.querySelector("#employeeStationExempt");
...
const customShiftSavePresetInput = document.querySelector("#customShiftSavePreset");
...
const cellEditorExpandToggleButton = document.querySelector("#cellEditorExpandToggle");
...
const cellEditorCoveragePanel = document.querySelector("#cellEditorCoveragePanel");
...
const cellEditorStationSetupButton = document.querySelector("#cellEditorStationSetup");
...
const openAutoFillOptionsButton = document.querySelector("#openAutoFillOptions");
const autoFillOptionsModal = document.querySelector("#autoFillOptionsModal");
const autoFillOpenerInput = document.querySelector("#autoFillOpener");
const autoFillClosingLeadInput = document.querySelector("#autoFillClosingLead");
const autoFillNightDropInput = document.querySelector("#autoFillNightDrop");
const autoFillDriveThruInput = document.querySelector("#autoFillDriveThru");
const autoFillOptionsCloseButton = document.querySelector("#autoFillOptionsClose");
const backToBranchRulesFromAutoFillButton = document.querySelector("#backToBranchRulesFromAutoFill");
const autoFillStationsButton = document.querySelector("#autoFillStations");
const openStationSetupButton = document.querySelector("#openStationSetup");
const stationAutoFillModal = document.querySelector("#stationAutoFillModal");
const stationAutoFillStatus = document.querySelector("#stationAutoFillStatus");
const stationAutoFillLocations = document.querySelector("#stationAutoFillLocations");
const stationAutoFillAddStationButton = document.querySelector("#stationAutoFillAddStation");
const stationAutoFillAddOfficeButton = document.querySelector("#stationAutoFillAddOffice");
const stationAutoFillLobbyMinutesInput = document.querySelector("#stationAutoFillLobbyMinutes");
const stationAutoFillRandomizeInput = document.querySelector("#stationAutoFillRandomize");
const stationAutoFillCancelButton = document.querySelector("#stationAutoFillCancel");
const stationAutoFillSaveButton = document.querySelector("#stationAutoFillSave");
const stationAutoFillApplyButton = document.querySelector("#stationAutoFillApply");
const backToSetupFromStationButton = document.querySelector("#backToSetupFromStation");
const backToBranchRulesFromStationButton = document.querySelector("#backToBranchRulesFromStation");
const branchRulesModal = document.querySelector("#branchRulesModal");
const branchShiftInput = document.querySelector("#branchShiftInput");
const branchShiftAddButton = document.querySelector("#branchShiftAdd");
const branchShiftStatus = document.querySelector("#branchShiftStatus");
const branchShiftList = document.querySelector("#branchShiftList");
const branchPartTimeShiftInput = document.querySelector("#branchPartTimeShiftInput");
const branchPartTimeShiftAddButton = document.querySelector("#branchPartTimeShiftAdd");
const branchPartTimeShiftStatus = document.querySelector("#branchPartTimeShiftStatus");
const branchPartTimeShiftList = document.querySelector("#branchPartTimeShiftList");
const branchWeekdayOpenTimeInput = document.querySelector("#branchWeekdayOpenTime");
const branchWeekdayCloseTimeInput = document.querySelector("#branchWeekdayCloseTime");
const branchSaturdayOpenTimeInput = document.querySelector("#branchSaturdayOpenTime");
const branchSaturdayCloseTimeInput = document.querySelector("#branchSaturdayCloseTime");
const branchHoursStatus = document.querySelector("#branchHoursStatus");
const branchOpenDayInputs = Array.from(document.querySelectorAll(".branch-open-day"));
const branchRuleTabs = Array.from(document.querySelectorAll(".branch-rule-tab"));
const branchRulePanels = Array.from(document.querySelectorAll(".branch-rule-panel"));
const branchRotationWeekCountInput = document.querySelector("#branchRotationWeekCount");
const branchRulesEmployeeSetupButton = document.querySelector("#branchRulesEmployeeSetup");
const branchEmployeeList = document.querySelector("#branchEmployeeList");
const branchStationStatus = document.querySelector("#branchStationStatus");
const branchStationLocations = document.querySelector("#branchStationLocations");
const branchStationAddStationButton = document.querySelector("#branchStationAddStation");
const branchStationAddOfficeButton = document.querySelector("#branchStationAddOffice");
const branchStationLobbyMinutesInput = document.querySelector("#branchStationLobbyMinutes");
const branchStationRandomizeInput = document.querySelector("#branchStationRandomize");
const branchStationExcludeManagersInput = document.querySelector("#branchStationExcludeManagers");
const branchStationSaveButton = document.querySelector("#branchStationSave");
const branchStationApplyButton = document.querySelector("#branchStationApply");
const branchRulesAutoFillOptionsButton = document.querySelector("#branchRulesAutoFillOptions");
const branchMinimumClosingStaffInput = document.querySelector("#branchMinimumClosingStaff");
const branchTaskRulesStatus = document.querySelector("#branchTaskRulesStatus");
const branchRulesCreateTemplateButton = document.querySelector("#branchRulesCreateTemplate");
const branchRulesCloseButton = document.querySelector("#branchRulesClose");
const backToSetupFromBranchRulesButton = document.querySelector("#backToSetupFromBranchRules");
const firstTimeSetupModal = document.querySelector("#firstTimeSetupModal");
const firstTimeSetupCloseButton = document.querySelector("#firstTimeSetupClose");
const setupGuideAddEmployeesButton = document.querySelector("#setupGuideAddEmployees");
const setupGuideBranchRulesButton = document.querySelector("#setupGuideBranchRules");
const setupGuideStationSetupButton = document.querySelector("#setupGuideStationSetup");
const setupGuideCreateTemplateButton = document.querySelector("#setupGuideCreateTemplate");
...
let firstTimeSetupFlowActive = false;
let branchRulesReturnActive = false;
let employeeBranchRulesReturnActive = false;
let branchRulesReturnTab = "times";
...
function initialize() {
  ...
  loadInitialState();
  renderWeekTypeOptions();
  renderEmployeeSaturdayRotationOptions();
...
function bindEvents() {
  ...
  weekTypeInput.addEventListener("change", async (event) => {
    const accepted = await askForConfirmation({
      ...
      message: "This will rebuild the week template. Regular weeks use the employee Saturday rotation rules, while Custom leaves Saturday open for manual planning.",
  ...
  async function createBranchTemplateExport() {
    closeMoreMenu();
    ...
    const accepted = await askForConfirmation({
      ...
      message: "This exports employees, roles, target hours, fixed days off, this branch's Saturday rotation weeks, branch shift times, auto-fill options, station settings, and custom task definitions. Weekly notes, helper rows, PTO entries, colors, day events, and task assignments will be left out.",
  ...
  openBranchRulesButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = false;
    openBranchRulesModal("times");
  });

  openAutoFillOptionsButton?.addEventListener("click", () => {
    closeMoreMenu();
    clearBranchRulesReturn();
    syncAutoFillOptionsModal();
    openModal(autoFillOptionsModal);
  });
  ...
  autoFillCoverageButton.addEventListener("click", async () => {
    if (!requireBranchShiftRulesForAutoFill()) {
      return;
    }
  ...
  autoFillOptionsCloseButton?.addEventListener("click", () => {
    if (branchRulesReturnActive) {
      closeModal(autoFillOptionsModal);
      returnToBranchRulesFromNested();
      return;
    }
    closeModal(autoFillOptionsModal);
  });

  [autoFillOpenerInput, autoFillClosingLeadInput, autoFillNightDropInput, autoFillDriveThruInput].forEach((input) => {
    input?.addEventListener("change", () => {
      state.autoFillSettings = getAutoFillSettingsFromInputs();
      setDirtyAndAutosave("Auto-fill options updated.");
      renderCoverage();
      renderSaveStatus();
    });
  });

  autoFillStationsButton?.addEventListener("click", () => {
    syncStationAutoFillModal();
    openModal(stationAutoFillModal);
  });

  openStationSetupButton?.addEventListener("click", () => {
    closeMoreMenu();
    firstTimeSetupFlowActive = false;
    clearBranchRulesReturn();
    syncSetupReturnButtons();
    syncStationAutoFillModal();
    openModal(stationAutoFillModal);
  });

  cellEditorStationSetupButton?.addEventListener("click", () => {
    clearBranchRulesReturn();
    syncStationAutoFillModal();
    openModal(stationAutoFillModal);
  });

  stationAutoFillCancelButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = false;
    clearBranchRulesReturn();
    syncSetupReturnButtons();
    closeModal(stationAutoFillModal);
  });

  stationAutoFillSaveButton?.addEventListener("click", () => {
    saveStationSetupRules();
    firstTimeSetupFlowActive = false;
    clearBranchRulesReturn();
    syncSetupReturnButtons();
    closeModal(stationAutoFillModal);
  ...
  stationAutoFillAddStationButton?.addEventListener("click", () => {
    addStationLocation("station", getStationSetupElements("modal"));
  });

  stationAutoFillAddOfficeButton?.addEventListener("click", () => {
    addStationLocation("office", getStationSetupElements("modal"));
  });

  stationAutoFillApplyButton?.addEventListener("click", () => {
    saveStationSetupRules();
    firstTimeSetupFlowActive = false;
    clearBranchRulesReturn();
    syncSetupReturnButtons();
  ...
  branchStationAddStationButton?.addEventListener("click", () => {
    addStationLocation("station", getStationSetupElements("branch"));
    saveStationSetupRules(getStationSetupElements("branch"), { silent: true });
  });

  branchStationAddOfficeButton?.addEventListener("click", () => {
    addStationLocation("office", getStationSetupElements("branch"));
    saveStationSetupRules(getStationSetupElements("branch"), { silent: true });
  });

  branchStationSaveButton?.addEventListener("click", () => {
    saveStationSetupRules(getStationSetupElements("branch"));
  });

  branchStationApplyButton?.addEventListener("click", () => {
    saveStationSetupRules(getStationSetupElements("branch"));
    autoFillStationPlan();
  });

  branchStationLobbyMinutesInput?.addEventListener("change", () => {
    saveStationSetupRules(getStationSetupElements("branch"), { silent: true });
  });

  branchStationRandomizeInput?.addEventListener("change", () => {
    saveStationSetupRules(getStationSetupElements("branch"), { silent: true });
  });

  branchStationExcludeManagersInput?.addEventListener("change", () => {
    saveStationSetupRules(getStationSetupElements("branch"), { silent: true });
  });

  branchShiftAddButton?.addEventListener("click", () => {
    addBranchShiftFromInput("full_time");
  });

  branchShiftInput?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
  ...
  branchPartTimeShiftAddButton?.addEventListener("click", () => {
    addBranchShiftFromInput("part_time");
  });

  branchPartTimeShiftInput?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      addBranchShiftFromInput("part_time");
    }
  ...
  [
    branchWeekdayOpenTimeInput,
    branchWeekdayCloseTimeInput,
    branchSaturdayOpenTimeInput,
    branchSaturdayCloseTimeInput,
  ].forEach((input) => {
    input?.addEventListener("blur", () => {
      applyBranchRulesAutomatically("Branch hours updated.");
    });
  ...
  branchMinimumClosingStaffInput?.addEventListener("change", () => {
    applyBranchRulesAutomatically("Closing staff rule updated.");
  });

  branchOpenDayInputs.forEach((input) => {
    input.addEventListener("change", () => {
      applyBranchRulesAutomatically("Open days updated.");
    });
  });

  branchRotationWeekCountInput?.addEventListener("change", () => {
    applyBranchRulesAutomatically("Rotation weeks updated.");
  });

  branchRuleTabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      setBranchRulesTab(tab.dataset.branchRuleTab);
    });
  });

  branchRulesEmployeeSetupButton?.addEventListener("click", () => {
    saveBranchRules({ silent: true });
    closeModal(branchRulesModal);
    openEmployeeEditorFromBranchRules();
  });

  branchRulesAutoFillOptionsButton?.addEventListener("click", () => {
    saveBranchRules({ silent: true });
    prepareBranchRulesReturn("tasks");
    closeModal(branchRulesModal);
    syncAutoFillOptionsModal();
    openModal(autoFillOptionsModal);
  });

  branchRulesCreateTemplateButton?.addEventListener("click", () => {
    saveBranchRules({ silent: true });
    firstTimeSetupFlowActive = false;
    syncSetupReturnButtons();
    closeModal(branchRulesModal);
    void createBranchTemplateExport();
  });

  branchRulesCloseButton?.addEventListener("click", () => {
    saveBranchRules({ silent: true });
    firstTimeSetupFlowActive = false;
    syncSetupReturnButtons();
    closeModal(branchRulesModal);
  ...
  firstTimeSetupCloseButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = false;
    syncSetupReturnButtons();
    closeModal(firstTimeSetupModal);
  });

  setupGuideAddEmployeesButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = true;
    syncSetupReturnButtons();
    closeModal(firstTimeSetupModal);
  ...
  setupGuideBranchRulesButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = true;
    syncSetupReturnButtons();
    closeModal(firstTimeSetupModal);
    openBranchRulesModal("rotation");
  });

  setupGuideStationSetupButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = true;
    syncSetupReturnButtons();
    closeModal(firstTimeSetupModal);
    openBranchRulesModal("stations");
  });

  setupGuideCreateTemplateButton?.addEventListener("click", () => {
    firstTimeSetupFlowActive = false;
    syncSetupReturnButtons();
    closeModal(firstTimeSetupModal);
    void createBranchTemplateExport();
  ...
  backToSetupFromEmployeesButton?.addEventListener("click", () => {
    returnToFirstTimeSetup();
  });

  backToBranchRulesFromEmployeesButton?.addEventListener("click", () => {
    returnToBranchRulesFromEmployees();
  });

  backToSetupFromBranchRulesButton?.addEventListener("click", () => {
    saveBranchRules({ silent: true });
    closeModal(branchRulesModal);
    returnToFirstTimeSetup();
  });

  backToSetupFromStationButton?.addEventListener("click", () => {
    saveStationSetupRules();
    closeModal(stationAutoFillModal);
    returnToFirstTimeSetup();
  });

  backToBranchRulesFromStationButton?.addEventListener("click", () => {
    saveStationSetupRules();
    closeModal(stationAutoFillModal);
    returnToBranchRulesFromNested();
  });

  backToBranchRulesFromAutoFillButton?.addEventListener("click", () => {
    closeModal(autoFillOptionsModal);
    returnToBranchRulesFromNested();
  });
  ...
  toggleEmployeeFormButton.addEventListener("click", () => {
    employeeBranchRulesReturnActive = false;
    syncSetupReturnButtons();
    resetEmployeeForm();
  ...
  cancelEmployeeEditButton.addEventListener("click", () => {
    const shouldReturnToBranchRules = employeeBranchRulesReturnActive;
    resetEmployeeForm();
    if (shouldReturnToBranchRules) {
      returnToBranchRulesFromEmployees();
    }
  });
  ...
  employeeEmploymentTypeInput.addEventListener("change", () => {
    ...
    if (employeeShiftPoolTypeInput.value === "auto") {
      renderSaveStatus();
    }
  });
  ...
  cellEditorExpandToggleButton?.addEventListener("click", (event) => {
    event.stopPropagation();
    state.cellEditorExpanded = !state.cellEditorExpanded;
    setDirtyAndAutosave("Cell editor size updated.");
    renderCellEditor();
  });
  ...
  autoFillOptionsModal?.addEventListener("click", (event) => {
    if (event.target === autoFillOptionsModal) {
      clearBranchRulesReturn();
      closeModal(autoFillOptionsModal);
    }
  });

  stationAutoFillModal?.addEventListener("click", (event) => {
    if (event.target === stationAutoFillModal) {
      firstTimeSetupFlowActive = false;
      clearBranchRulesReturn();
      syncSetupReturnButtons();
      closeModal(stationAutoFillModal);
  ...
  branchRulesModal?.addEventListener("click", (event) => {
    if (event.target === branchRulesModal) {
      saveBranchRules({ silent: true });
      firstTimeSetupFlowActive = false;
      syncSetupReturnButtons();
      closeModal(branchRulesModal);
  ...
  firstTimeSetupModal?.addEventListener("click", (event) => {
    if (event.target === firstTimeSetupModal) {
      firstTimeSetupFlowActive = false;
      syncSetupReturnButtons();
      closeModal(firstTimeSetupModal);
    }
...
function isInactiveShift(shift) {
  return ["OFF", "PTO", holidayShift, darkCellShift, closedCellShift, helperPlaceholderShift].includes(shift);
}
...
function getDisplayShiftLabel(shift) {
  ...
  if (shift === holidayShift) {
    return "Holiday";
  }
  return shift === closedCellShift ? "Closed" : shift;
}
...
function render() {
  ...
  scheduleRange.textContent = `${state.currentWeekType} • ${formatLongDate(dates[0])} - ${formatLongDate(dates[dates.length - 1])}`;
...
function renderSaveStatus() {
  ...
  if (state.fileBackupNeeded) {
    parts.push("Save Weekly File recommended before closing");
  }
...
function renderHeader(dates) {
  ...
  dates.forEach((date, index) => {
    ...
    if (dayName === "Sunday") {
      const sundayStatus = document.createElement("span");
      sundayStatus.className = "sunday-status-label";
      sundayStatus.textContent = isBranchClosedDay("Sunday") ? "Branch closed • Event only" : "Manual event day";
      dayWrap.appendChild(sundayStatus);
    }
...
function renderBody() {
  ...
  employees.forEach((employee) => {
    ...
    weekDays.forEach((day, dayIndex) => {
      ...
      const stationLine = cell.querySelector(".cell-station-line");
      ...
      cell.classList.toggle("is-closed", assignment.shift === closedCellShift);
      ...
      renderCellStationLine(stationLine, assignment);
...
function renderCellEditor() {
  ...
  cellEditor.classList.toggle("is-expanded", state.cellEditorExpanded);
  cellEditor.classList.toggle("is-bulk-editing", bulkMode);
  if (cellEditorExpandToggleButton) {
    cellEditorExpandToggleButton.textContent = state.cellEditorExpanded ? "Compact" : "Expand";
    cellEditorExpandToggleButton.setAttribute(
      "aria-label",
      state.cellEditorExpanded ? "Use compact cell editor width" : "Expand cell editor width"
    );
  }
  ...
  if (cellEditorCoveragePanel) {
    cellEditorCoveragePanel.open = state.stationScheduleEnabled && hasAssignmentDetails(assignment);
  }
...
function renderBulkEditor() {
  ...
  syncCoverageDetailsSectionVisibility();
  if (cellEditorCoveragePanel) {
    cellEditorCoveragePanel.open = false;
  }
...
function syncAutoFillOptionsModal() {
  const settings = normalizeAutoFillSettings(state.autoFillSettings);
  autoFillOpenerInput.checked = settings.opener;
  autoFillClosingLeadInput.checked = settings.closingLead;
  autoFillNightDropInput.checked = settings.nightDrop;
  autoFillDriveThruInput.checked = settings.driveThru;
}

function getAutoFillSettingsFromInputs() {
  return normalizeAutoFillSettings({
    opener: autoFillOpenerInput.checked,
    closingLead: autoFillClosingLeadInput.checked,
    nightDrop: autoFillNightDropInput.checked,
    driveThru: autoFillDriveThruInput.checked,
    minimumClosingStaff: state.autoFillSettings?.minimumClosingStaff,
  });
}

function openBranchRulesModal(tabName = "times") {
  renderBranchShiftRules();
  syncBranchTaskRulesControls();
  renderBranchEmployeeList();
  syncBranchStationSetupPanel();
  if (branchRotationWeekCountInput) {
    branchRotationWeekCountInput.value = String(normalizeRotationWeekCount(state.rotationWeekCount));
  }
  setBranchRulesTab(tabName);
  syncSetupReturnButtons();
  syncBranchRulesReturnButtons();
  openModal(branchRulesModal);
}

function setBranchRulesTab(tabName) {
  const nextTab = branchRulePanels.some((panel) => panel.dataset.branchRulePanel === tabName)
    ? tabName
    : "times";

  branchRuleTabs.forEach((tab) => {
    const isActive = tab.dataset.branchRuleTab === nextTab;
    tab.classList.toggle("is-active", isActive);
    tab.setAttribute("aria-selected", String(isActive));
    tab.setAttribute("aria-expanded", String(isActive));
  });

  branchRulePanels.forEach((panel) => {
    panel.classList.toggle("is-active", panel.dataset.branchRulePanel === nextTab);
  });
}

function returnToFirstTimeSetup() {
  firstTimeSetupFlowActive = true;
  syncSetupReturnButtons();
  openModal(firstTimeSetupModal);
}

function returnToBranchRulesFromEmployees() {
  employeeBranchRulesReturnActive = false;
  firstTimeSetupFlowActive = false;
  resetEmployeeForm();
  syncSetupReturnButtons();
  openBranchRulesModal("employees");
}

function syncSetupReturnButtons() {
  backToSetupFromEmployeesButton?.classList.toggle("is-hidden", !firstTimeSetupFlowActive);
  backToBranchRulesFromEmployeesButton?.classList.toggle(
    "is-hidden",
    !employeeBranchRulesReturnActive || employeeForm.classList.contains("is-hidden")
  );
  backToSetupFromBranchRulesButton?.classList.toggle("is-hidden", !firstTimeSetupFlowActive);
  backToSetupFromStationButton?.classList.toggle("is-hidden", !firstTimeSetupFlowActive || branchRulesReturnActive);
}

function prepareBranchRulesReturn(tabName) {
  branchRulesReturnActive = true;
  branchRulesReturnTab = tabName || "times";
  syncBranchRulesReturnButtons();
  syncSetupReturnButtons();
}

function clearBranchRulesReturn() {
  branchRulesReturnActive = false;
  branchRulesReturnTab = "times";
  syncBranchRulesReturnButtons();
  syncSetupReturnButtons();
}

function syncBranchRulesReturnButtons() {
  [backToBranchRulesFromStationButton, backToBranchRulesFromAutoFillButton].forEach((button) => {
    button?.classList.toggle("is-hidden", !branchRulesReturnActive);
  });
}

function returnToBranchRulesFromNested() {
  const returnTab = branchRulesReturnTab || "times";
  branchRulesReturnActive = false;
  syncBranchRulesReturnButtons();
  syncSetupReturnButtons();
  openBranchRulesModal(returnTab);
}

function requireBranchShiftRulesForAutoFill() {
  if (getAllBranchShiftPresets().length > 0) {
    return true;
  }

  showToast("Add branch shift times before using auto fill.");
  openBranchRulesModal();
  return false;
}

function renderBranchShiftRules() {
  syncBranchHoursControls();
  renderBranchShiftGroup({
    list: branchShiftList,
    status: branchShiftStatus,
    shifts: getBranchShiftPresets("full_time"),
    type: "full_time",
    emptyMessage: "No full-time shift times yet. Add times like 8-5:45 or 8-6.",
    statusLabel: "full-time",
  });
  renderBranchShiftGroup({
    list: branchPartTimeShiftList,
    status: branchPartTimeShiftStatus,
    shifts: getBranchShiftPresets("part_time"),
    type: "part_time",
    emptyMessage: "No part-time shift times yet. Add times like 10-2 or 12-4.",
    statusLabel: "part-time",
  });
}

function syncBranchHoursControls() {
  const hours = normalizeBranchHours(state.branchHours);
  const closedDays = new Set(normalizeBranchClosedDays(state.branchClosedDays));
  if (branchWeekdayOpenTimeInput) {
    branchWeekdayOpenTimeInput.value = hours.weekdayOpen;
  }
  if (branchWeekdayCloseTimeInput) {
    branchWeekdayCloseTimeInput.value = hours.weekdayClose;
  }
  if (branchSaturdayOpenTimeInput) {
    branchSaturdayOpenTimeInput.value = hours.saturdayOpen;
  }
  if (branchSaturdayCloseTimeInput) {
    branchSaturdayCloseTimeInput.value = hours.saturdayClose;
  }
  branchOpenDayInputs.forEach((input) => {
    input.checked = !closedDays.has(input.value);
  });
  if (branchHoursStatus) {
    const regularClosedDays = [...closedDays].filter((day) => day !== "Sunday");
    const closedLabel = regularClosedDays.length
      ? ` • Closed: ${regularClosedDays.map((day) => day.slice(0, 3)).join(", ")}`
      : "";
    const sundayLabel = closedDays.has("Sunday") ? "Sunday closed (manual event only)" : "Sunday manual event day";
    branchHoursStatus.textContent = `Weekday ${hours.weekdayOpen}-${hours.weekdayClose} • Saturday ${hours.saturdayOpen}-${hours.saturdayClose} • ${sundayLabel}${closedLabel}`;
  }
}

function getBranchHoursFromInputs() {
  return normalizeBranchHours({
    weekdayOpen: branchWeekdayOpenTimeInput?.value,
    weekdayClose: branchWeekdayCloseTimeInput?.value,
    saturdayOpen: branchSaturdayOpenTimeInput?.value,
    saturdayClose: branchSaturdayCloseTimeInput?.value,
  });
}

function getBranchClosedDaysFromInputs() {
  const openDays = new Set(branchOpenDayInputs.filter((input) => input.checked).map((input) => input.value));
  return normalizeBranchClosedDays(weekDays.filter((day) => !openDays.has(day)));
}

function syncBranchTaskRulesControls() {
  const settings = normalizeAutoFillSettings(state.autoFillSettings);
  if (branchMinimumClosingStaffInput) {
    branchMinimumClosingStaffInput.value = String(settings.minimumClosingStaff);
  }
  if (branchTaskRulesStatus) {
    branchTaskRulesStatus.textContent = `Closing Lead requires ${settings.minimumClosingStaff} total closing staff.`;
  }
}

function getBranchTaskRulesFromInputs() {
  return {
    ...normalizeAutoFillSettings(state.autoFillSettings),
    minimumClosingStaff: normalizeMinimumClosingStaff(branchMinimumClosingStaffInput?.value),
  };
}

function renderBranchShiftGroup({ list, status, shifts, type, emptyMessage, statusLabel }) {
  if (!list || !status) {
    return;
  }
  ...
  list.innerHTML = "";
  if (shifts.length === 0) {
    status.textContent = `No ${statusLabel} shift times saved yet.`;
    list.appendChild(buildEmptyStateRow(emptyMessage));
    return;
  }

  status.textContent = `${shifts.length} ${statusLabel} shift times saved for this branch.`;
  shifts.forEach((shift) => {
    list.appendChild(buildBranchShiftRow(shift, type));
  });
}
...
function buildBranchShiftRow(shift, type = "full_time") {
  const row = document.createElement("div");
  row.className = "branch-rule-row";
  row.dataset.shiftType = type;
  row.draggable = true;
  const config = getBranchShiftGroupConfig(type);

  const dragHandle = document.createElement("button");
  dragHandle.type = "button";
  dragHandle.className = "branch-rule-drag-handle";
  dragHandle.textContent = "Drag";
  dragHandle.setAttribute("aria-label", `Drag ${shift} to reorder`);
  ...
  input.type = "text";
  input.value = shift;
  input.dataset.originalShift = shift;
  input.placeholder = "8-5:45";
  input.addEventListener("focus", () => {
    row.draggable = false;
  });
  input.addEventListener("blur", () => {
    row.draggable = true;
    const normalized = normalizeShiftRangeInput(input.value);
    if (!normalized) {
      input.value = input.dataset.originalShift;
      config.status.textContent = "Shift time restored. Use a valid time like 8-5:45.";
      return;
    }
    input.value = normalized;
    input.dataset.originalShift = normalized;
    applyBranchRulesAutomatically("Shift time updated.");
  });
  ...
  removeButton.addEventListener("click", () => {
    row.remove();
    if (config.list.querySelectorAll(".branch-rule-row").length === 0) {
      config.list.appendChild(buildEmptyStateRow(config.emptyMessage));
    }
    applyBranchRulesAutomatically("Shift time removed.");
  });
  ...
  row.addEventListener("dragend", () => {
    ...
    config.list.querySelectorAll(".branch-rule-row").forEach((entry) => {
  ...
  row.addEventListener("dragover", (event) => {
    event.preventDefault();
    const draggingRow = config.list.querySelector(".branch-rule-row.is-dragging");
    if (!draggingRow || draggingRow === row) {
      return;
    }

    const insertAfter = event.clientY > row.getBoundingClientRect().top + row.offsetHeight / 2;
    config.list.insertBefore(draggingRow, insertAfter ? row.nextSibling : row);
  ...
  row.addEventListener("drop", (event) => {
    ...
    applyBranchRulesAutomatically("Shift order updated.");
  });
...
function getBranchShiftGroupConfig(type = "full_time") {
  if (type === "part_time") {
    return {
      input: branchPartTimeShiftInput,
      list: branchPartTimeShiftList,
      status: branchPartTimeShiftStatus,
      emptyMessage: "No part-time shift times yet. Add times like 10-2 or 12-4.",
    };
...
function addBranchShiftFromInput(type = "full_time") {
  const config = getBranchShiftGroupConfig(type);
  const normalized = normalizeShiftRangeInput(config.input.value);
  if (!normalized) {
    config.status.textContent = "Enter a valid shift like 8-5:45 or 8:00-1:30.";
    return;
  }

  const current = getBranchShiftRulesFromModal(type);
  if (current.includes(normalized)) {
    config.status.textContent = `${normalized} is already saved.`;
    config.input.value = "";
    return;
  }

  config.list.querySelector(".setup-empty-state")?.remove();
  config.list.appendChild(buildBranchShiftRow(normalized, type));
  config.input.value = "";
  applyBranchRulesAutomatically(`${normalized} added.`);
}

function getBranchShiftRulesFromModal(type = "full_time") {
  const config = getBranchShiftGroupConfig(type);
  return normalizeBranchShiftPresets(
    Array.from(config.list.querySelectorAll(".branch-rule-row input"))
...
function getBranchShiftPresets(type = "full_time") {
  if (type === "part_time") {
    return normalizeBranchShiftPresets(state.branchPartTimeShiftPresets, []);
  }
  return normalizeBranchShiftPresets(state.branchShiftPresets, state.customShiftPresets);
}

function getAllBranchShiftPresets() {
  return normalizeBranchShiftPresets([...getBranchShiftPresets("full_time"), ...getBranchShiftPresets("part_time")], []);
}

function applyBranchRulesAutomatically(message = "Branch rules updated.") {
  saveBranchRules({ silent: true });
  if (branchShiftStatus) {
    branchShiftStatus.textContent = `${message} Applied automatically.`;
  }
  if (branchPartTimeShiftStatus) {
    branchPartTimeShiftStatus.textContent = `${message} Applied automatically.`;
  }
}

function saveBranchRules({ silent = false } = {}) {
  const previousRotationWeekCount = normalizeRotationWeekCount(state.rotationWeekCount);
  const nextRotationWeekCount =
    branchRotationWeekCountInput?.value === ""
      ? ""
      : normalizeRotationWeekCount(branchRotationWeekCountInput?.value, previousRotationWeekCount);
  const rotationCountChanged = previousRotationWeekCount !== nextRotationWeekCount;

  state.rotationWeekCount = nextRotationWeekCount;
  state.rotationWeekCountConfigured = nextRotationWeekCount !== "";
  renderWeekTypeOptions();
  renderEmployeeSaturdayRotationOptions();
  if (rotationCountChanged) {
    hydrateEmployees(employees);
    buildDefaultAssignments({ preserveAssignments: true });
  }
  state.branchShiftPresets = getBranchShiftRulesFromModal("full_time");
  state.branchPartTimeShiftPresets = getBranchShiftRulesFromModal("part_time");
  state.branchHours = getBranchHoursFromInputs();
  const previousClosedDays = normalizeBranchClosedDays(state.branchClosedDays);
  state.branchClosedDays = getBranchClosedDaysFromInputs();
  syncBranchHoursControls();
  state.autoFillSettings = getBranchTaskRulesFromInputs();
  syncBranchTaskRulesControls();
  const closedDaysChanged = previousClosedDays.join("|") !== state.branchClosedDays.join("|");
  if (closedDaysChanged) {
    applyClosedDayRules();
    buildDefaultAssignments({ preserveAssignments: true });
  }
  if (branchStationLocations) {
    state.stationAutoFillSettings = getStationAutoFillSettingsFromInputs(getStationSetupElements("branch"));
  }
  state.customShiftPresets = [];
  setDirtyAndAutosave(silent ? undefined : "Branch rules saved.");
  if (rotationCountChanged || closedDaysChanged) {
    render();
  } else {
    renderCellEditor();
    renderSaveStatus();
  ...
  if (!silent) {
    renderBranchShiftRules();
    syncBranchStationSetupPanel();
    showToast("Branch rules saved.");
  }
}

function syncStationAutoFillModal() {
  syncStationAutoFillControls(getStationSetupElements("modal"));
}

function syncBranchStationSetupPanel() {
  syncStationAutoFillControls(getStationSetupElements("branch"));
}

function getStationSetupElements(source = "modal") {
  if (source === "branch") {
    return {
      status: branchStationStatus,
      locations: branchStationLocations,
      lobbyMinutes: branchStationLobbyMinutesInput,
      randomize: branchStationRandomizeInput,
      excludeManagers: branchStationExcludeManagersInput,
    };
  }

  return {
    status: stationAutoFillStatus,
    locations: stationAutoFillLocations,
    lobbyMinutes: stationAutoFillLobbyMinutesInput,
    randomize: stationAutoFillRandomizeInput,
    excludeManagers: null,
  };
}

function syncStationAutoFillControls(elements = getStationSetupElements("modal")) {
  if (!elements.locations) {
    return;
  }
  const settings = normalizeStationAutoFillSettings(state.stationAutoFillSettings);
  elements.lobbyMinutes.value = String(settings.lobbyLeadMinutes);
  elements.randomize.checked = settings.randomize;
  if (elements.excludeManagers) {
    elements.excludeManagers.checked = settings.excludeManagers !== false;
  }
  elements.status.textContent = settings.locations.length
    ? `${settings.locations.length} stations/offices set up. Click Save Setup to keep changes.`
    : "No stations or offices yet. Add Station or Add Office to build this branch setup.";
  renderStationLocationRows(settings.locations, elements);
}

function getStationAutoFillSettingsFromInputs(elements = getStationSetupElements("modal")) {
  return normalizeStationAutoFillSettings({
    locations: getStationLocationsFromInputs(elements),
    randomize: elements.randomize.checked,
    lobbyLeadMinutes: elements.lobbyMinutes.value,
    excludeManagers: elements.excludeManagers ? elements.excludeManagers.checked : state.stationAutoFillSettings?.excludeManagers !== false,
  });
}

function renderStationLocationRows(locations = [], elements = getStationSetupElements("modal")) {
  if (!elements.locations) {
    return;
  }
  elements.locations.innerHTML = "";
  if (locations.length === 0) {
    elements.locations.appendChild(buildEmptyStateRow("No stations/offices yet. Add the locations this branch uses."));
    return;
  }

  locations.forEach((location) => {
    elements.locations.appendChild(buildStationLocationRow(location, elements));
  });
}

function buildStationLocationRow(location, elements = getStationSetupElements("modal")) {
  const row = document.createElement("div");
  row.className = `station-location-row is-${location.type}`;
  row.dataset.id = location.id;
  row.dataset.type = location.type;

  const nameCell = document.createElement("div");
  nameCell.className = "station-location-name-cell";
  const typePill = document.createElement("span");
  typePill.className = "station-location-type";
  typePill.textContent = location.type === "office" ? "Office" : "Station";

  const labelInput = document.createElement("input");
  labelInput.type = "text";
  labelInput.className = "station-location-name";
  labelInput.value = location.label;
  labelInput.placeholder = location.type === "office" ? "Office 1" : "Station 1";
  nameCell.appendChild(typePill);
  nameCell.appendChild(labelInput);

  const lunchInput = document.createElement("input");
  lunchInput.type = "text";
  lunchInput.className = "station-location-lunch";
  lunchInput.value = location.lunch || "";
  lunchInput.placeholder = "12:00";

  const removeButton = document.createElement("button");
  removeButton.type = "button";
  removeButton.className = "secondary-button station-location-remove";
  removeButton.textContent = "Remove";
  removeButton.addEventListener("click", () => {
    row.remove();
    if (elements.locations.querySelectorAll(".station-location-row").length === 0) {
      elements.locations.appendChild(buildEmptyStateRow("No stations/offices yet. Add the locations this branch uses."));
    }
    updateStationSetupFeedback(`${location.label} removed.`, elements);
    saveStationSetupRules(elements, { silent: true });
  });

  [labelInput, lunchInput].forEach((input) => {
    input.addEventListener("blur", () => {
      saveStationSetupRules(elements, { silent: true });
    });
  });
  ...
  row.appendChild(lunchInput);
  row.appendChild(removeButton);
  return row;
}

function addStationLocation(type, elements = getStationSetupElements("modal")) {
  const current = getStationLocationsFromInputs(elements);
  const nextCount = current.filter((location) => location.type === type).length + 1;
  const label = `${type === "office" ? "Office" : "Station"} ${nextCount}`;
  const location = {
    id: `location-${Date.now()}-${Math.random().toString(36).slice(2, 5)}`,
    type,
    label,
    lunch: "",
  };
  elements.locations.querySelector(".setup-empty-state")?.remove();
  elements.locations.appendChild(buildStationLocationRow(location, elements));
  updateStationSetupFeedback(`${label} added. Add a lunch time if this location has a set lunch.`, elements);
}

function getStationLocationsFromInputs(elements = getStationSetupElements("modal")) {
  return Array.from(elements.locations.querySelectorAll(".station-location-row"))
    .map((row) => ({
      id: row.dataset.id,
      type: row.dataset.type === "office" ? "office" : "station",
      label: row.querySelector(".station-location-name")?.value.trim() || "",
      lunch: normalizeShiftTimeLabel(row.querySelector(".station-location-lunch")?.value || "") || "",
    }))
    .filter((location) => location.label);
}

function saveStationSetupRules(elements = getStationSetupElements("modal"), { silent = false } = {}) {
  state.stationAutoFillSettings = getStationAutoFillSettingsFromInputs(elements);
  setDirtyAndAutosave("Station setup saved.");
  renderSaveStatus();
  renderWorkspaceTabs();
  elements.status.textContent = state.stationAutoFillSettings.locations.length
    ? `${state.stationAutoFillSettings.locations.length} stations/offices saved.`
    : "Station setup saved with no stations/offices.";
  renderStationLocationRows(state.stationAutoFillSettings.locations, elements);
  if (!silent) {
    showToast("Station setup saved.");
  }
}

function updateStationSetupFeedback(message, elements = getStationSetupElements("modal")) {
  const count = getStationLocationsFromInputs(elements).length;
  elements.status.textContent = `${message} ${count} total stations/offices.`;
}
...
function renderCellStationLine(stationRoot, assignment) {
  if (!stationRoot) {
    return;
  }

  stationRoot.innerHTML = "";
  const station = state.stationScheduleEnabled ? String(assignment.station || "").trim() : "";
  if (!station || [darkCellShift, closedCellShift].includes(assignment.shift)) {
    stationRoot.classList.add("is-empty");
    return;
  }

  stationRoot.classList.remove("is-empty");
  const stationTag = document.createElement("span");
  stationTag.className = "station-cell-tag";
  stationTag.textContent = station;
  stationRoot.appendChild(stationTag);
}
...
function autoFillStationPlan() {
  applyClosedDayRules();
  const settings = normalizeStationAutoFillSettings(state.stationAutoFillSettings);
  const configuredLocations = settings.locations;
  if (configuredLocations.length === 0) {
    showToast("Add stations/offices before auto filling the Station Plan.");
    syncStationAutoFillModal();
    openModal(stationAutoFillModal);
    return;
  }
  ...
  state.stationScheduleEnabled = true;
  if (stationScheduleToggle) {
    stationScheduleToggle.checked = true;
  }

  autoFillWeekDays.forEach((day) => {
    const scheduledRows = getScheduledEmployeesForDay(day).filter(({ employee }) => !employee.assistRow);
    scheduledRows.forEach(({ assignment }) => {
      assignment.station = "";
      assignment.lunch = "";
      assignment.lobbyLead = "";
    });

    const eligibleRows = scheduledRows
      .filter(({ employee }) => isEmployeeEligibleForStationAssignment(employee, settings))
      .sort((left, right) => (getShiftStart(left.assignment.shift) ?? 0) - (getShiftStart(right.assignment.shift) ?? 0));
    const locationPool = settings.randomize
      ? shuffleArray(configuredLocations, `${day}-${runSeed}`)
      : configuredLocations;

    eligibleRows.slice(0, locationPool.length).forEach(({ employee, assignment }, index) => {
      const location = locationPool[index] || null;
      assignment.station = location?.label || "";

      const start = getShiftStart(assignment.shift);
      const end = getShiftEnd(assignment.shift);
      const shiftHours = getShiftHours(assignment.shift, employee);
      assignment.lunch = start !== null && end !== null && shiftHours >= 6 ? location?.lunch || "" : "";

      filledCount += 1;
    });
  });

  setDirtyAndAutosave("Station plan auto-filled.");
  render();
  showToast(`Auto-filled station details for ${filledCount} scheduled cells.`);
}
...
function renderCoverage() {
  ...
  weekDays.forEach((day) => {
    if (day === "Sunday") {
      const totals = getCoverageTotalsForDay(day);
      const card = document.createElement("article");
      card.className = "coverage-item is-ok manual-only-coverage";
      card.innerHTML = `
        <strong>Sunday</strong>
        <div>Staffed: ${totals.staffed}</div>
        <div>${isBranchClosedDay("Sunday") ? "Branch closed by default" : "Manual event day enabled"}</div>
        <div>Manual event scheduling only</div>
        <div class="coverage-ok">No auto-fill coverage required</div>
      `;
      coverageSummary.appendChild(card);
      return;
    }

    if (isBranchClosedDay(day)) {
      const card = document.createElement("article");
      card.className = "coverage-item is-ok";
      card.innerHTML = `
        <strong>${day}</strong>
        <div>Branch closed</div>
        <div class="coverage-ok">Closed</div>
      `;
      coverageSummary.appendChild(card);
      return;
    }
    ...
    const closingStaffCount = getClosingStaffCount(day);
    ...
    card.innerHTML = `
      ...
      <div>Opener: ${isHolidayDay || !state.autoFillSettings.opener ? "Complete" : totals.opener}</div>
      <div>Closing Lead: ${isHolidayDay || !state.autoFillSettings.closingLead ? "Complete" : totals.closingLead}</div>
      <div>Closing Staff: ${isHolidayDay || !state.autoFillSettings.closingLead ? "Complete" : `${closingStaffCount}/${getMinimumClosingStaff()}`}</div>
      <div>Night Drop: ${isHolidayDay || !state.autoFillSettings.nightDrop ? "Complete" : `${totals.nightDrop}/2`}</div>
      <div>Drive Thru: ${isHolidayDay || !state.autoFillSettings.driveThru ? "Complete" : `${totals.driveThru}/1`}</div>
...
function getDailyCoverageWarnings(day, totals) {
  if (day === "Sunday" || isBranchClosedDay(day)) {
    return [];
  }
  ...
  if (state.autoFillSettings.opener && totals.opener < 1) {
    warnings.push("Missing opener");
  } else if (state.autoFillSettings.opener) {
  ...
  if (state.autoFillSettings.closingLead && totals.closingLead < 1) {
    warnings.push("Missing closing lead");
  }
  if (state.autoFillSettings.closingLead && getClosingStaffCount(day) < getMinimumClosingStaff()) {
    warnings.push(`Closing needs ${getMinimumClosingStaff()} total`);
  }
  if (state.autoFillSettings.nightDrop && totals.nightDrop < builtInTaskConfig.nightDrop.dailyRequired) {
    warnings.push(`Night Drop needs ${builtInTaskConfig.nightDrop.dailyRequired}`);
  }
  if (state.autoFillSettings.driveThru && totals.driveThru < builtInTaskConfig.driveThru.dailyRequired) {
...
function getClosingStaffCount(day) {
  const closingTime = getCoverageClosingTime(day);
  return getScheduledEmployeesForDay(day).filter(
    ({ assignment }) => (getShiftEnd(assignment.shift) ?? 0) >= closingTime
  ).length;
}

function getMinimumClosingStaff() {
  return normalizeAutoFillSettings(state.autoFillSettings).minimumClosingStaff;
}
...
function renderEmployeeRoster() {
  ...
  employees.forEach((employee) => {
    ...
    const taskExemptionText = getTaskExemptionSummary(employee);
    ...
    card.innerHTML = `
      ...
      ${taskExemptionText ? `<p class="task-exemption-summary">${taskExemptionText}</p>` : ""}
      ${actionsMarkup}
    `;
...
function renderBranchEmployeeList() {
  if (!branchEmployeeList) {
    return;
  }

  branchEmployeeList.innerHTML = "";
  const regularEmployees = employees.filter((employee) => !employee.assistRow);

  if (regularEmployees.length === 0) {
    branchEmployeeList.innerHTML = `
      <div class="branch-employee-empty">
        <strong>No employees yet</strong>
        <p>Open the employee editor to add the branch team.</p>
      </div>
    `;
    return;
  }
  ...
  regularEmployees.forEach((employee) => {
    ...
    row.className = "branch-employee-row";
    row.innerHTML = `
      <strong>${escapeHtml(employee.name)}</strong>
      <span>${escapeHtml(employee.idNumber || "N/A")}</span>
      <span>${escapeHtml(employee.drawerNumber || "N/A")}</span>
      <div class="branch-employee-actions">
        <button type="button" class="secondary-button" data-action="edit">Edit</button>
        <button type="button" class="secondary-button danger-lite-button" data-action="remove">Remove</button>
      </div>
    `;
    ...
    row.querySelector('[data-action="edit"]')?.addEventListener("click", () => {
      saveBranchRules({ silent: true });
      closeModal(branchRulesModal);
      openEmployeeEditorFromBranchRules(employee);
    });
    ...
    branchEmployeeList.appendChild(row);
  });
}

function openEmployeeEditorFromBranchRules(employee = null) {
  employeeBranchRulesReturnActive = true;
  firstTimeSetupFlowActive = false;
...
function hasRotationWeekCount(value) {
  const numeric = Number(value);
  return Number.isFinite(numeric) && numeric >= 2 && numeric <= 8;
}

function normalizeRotationWeekCount(value, fallback = defaultRotationWeekCount) {
  if (value === "" || value === null || value === undefined) {
    return fallback === "" ? "" : normalizeRotationWeekCount(fallback, fallbackRotationWeekCount);
  }
  const numeric = Number(value);
  if (Number.isFinite(numeric)) {
    return Math.min(8, Math.max(2, Math.round(numeric)));
  }
  return fallback === "" ? "" : Math.min(8, Math.max(2, fallback));
}

function getActiveWeekTypeOptions() {
  const count = normalizeRotationWeekCount(state.rotationWeekCount);
  return count ? weekTypeOptions.slice(0, count) : [];
}

function getWeekLetter(weekType) {
  return String(weekType || "").replace("Week ", "");
}

function getWeekIndex(weekType) {
  const index = weekTypeOptions.indexOf(weekType);
  return index >= 0 ? index + 1 : 0;
}

function inferRotationWeekCount(payload = {}, employeeList = []) {
  if (payload.rotationWeekCount !== undefined) {
    return normalizeRotationWeekCount(payload.rotationWeekCount);
  }

  const discoveredIndexes = [getWeekIndex(payload.currentWeekType)];
  employeeList.forEach((employee) => {
    const numericGroup = Number(employee.saturdayGroup);
    if (Number.isFinite(numericGroup)) {
      discoveredIndexes.push(numericGroup);
    }
    employee.saturdayRule?.workWeeks?.forEach((weekType) => {
      discoveredIndexes.push(getWeekIndex(weekType));
    });
  });

  return normalizeRotationWeekCount(Math.max(fallbackRotationWeekCount, ...discoveredIndexes));
}

function renderWeekTypeOptions() {
  const previousValue = state.currentWeekType || weekTypeInput.value || "Custom";
  const activeWeeks = getActiveWeekTypeOptions();
  weekTypeInput.innerHTML = "";

  if (activeWeeks.length === 0) {
    const placeholder = document.createElement("option");
    placeholder.value = "Custom";
    placeholder.textContent = "Set rotation in Branch Rules";
    weekTypeInput.appendChild(placeholder);
    state.currentWeekType = "Custom";
    weekTypeInput.value = state.currentWeekType;
    return;
  }

  [...activeWeeks, "Custom"].forEach((weekType) => {
    const option = document.createElement("option");
    option.value = weekType;
    option.textContent = weekType;
    weekTypeInput.appendChild(option);
  });

  state.currentWeekType = [...activeWeeks, "Custom"].includes(previousValue) ? previousValue : "Week A";
  weekTypeInput.value = state.currentWeekType;
}

function renderEmployeeSaturdayRotationOptions() {
  const previousValue = employeeSaturdayGroupInput.value || "1";
  const activeWeeks = getActiveWeekTypeOptions();
  const oddWeeks = activeWeeks.filter((_, index) => index % 2 === 0).map(getWeekLetter).join("/");
  const evenWeeks = activeWeeks.filter((_, index) => index % 2 === 1).map(getWeekLetter).join("/");

  employeeSaturdayGroupInput.innerHTML = "";
  if (activeWeeks.length === 0) {
    const placeholder = document.createElement("option");
    placeholder.value = "";
    placeholder.textContent = "Set rotation weeks in Branch Rules first";
    employeeSaturdayGroupInput.appendChild(placeholder);
    employeeSaturdayGroupInput.value = "";
    return;
  }

  [
    { value: "0", label: "Works Every Rotating Saturday" },
    { value: "odd", label: `Pair Rotation: ${oddWeeks || "A"} Off` },
    { value: "even", label: `Pair Rotation: ${evenWeeks || "B"} Off` },
    ...activeWeeks.map((weekType, index) => ({
      value: String(index + 1),
      label: `Cycle Rotation: ${weekType} Off`,
    })),
  ].forEach(({ value, label }) => {
    const option = document.createElement("option");
    option.value = value;
    option.textContent = label;
    employeeSaturdayGroupInput.appendChild(option);
  });

  employeeSaturdayGroupInput.value =
    ["0", "odd", "even"].includes(previousValue) || Number(previousValue) <= activeWeeks.length
      ? previousValue
      : "1";
}
...
function resetEmployeeForm() {
  ...
  employeeTaskExemptionOptions.forEach((option) => {
  ...
  employeeShiftPoolTypeInput.value = "auto";
  employeeTargetHoursInput.value = "40";
  employeeStationExemptInput.checked = false;
  syncCustomKeyField();
  syncEmployeeRowTypeFields();
  syncTargetHoursField();
...
function loadEmployeeIntoForm(employee) {
  ...
  employeeTaskExemptionOptions.forEach((option) => {
    option.checked = isEmployeeExemptFromTask(employee, option.value);
  });
  ...
  employeeSaturdayGroupInput.value = Array.from(employeeSaturdayGroupInput.options).some(
    (option) => option.value === String(employee.saturdayGroup)
  )
    ? String(employee.saturdayGroup)
    : "1";
  employeeEmploymentTypeInput.value = employee.assistRow ? "full_time" : employee.employmentType || "full_time";
  employeeShiftPoolTypeInput.value = employee.assistRow ? "auto" : employee.shiftPoolType || "auto";
  employeeTargetHoursInput.value = String(employee.assistRow ? 0 : employee.targetHours ?? 40);
  employeeStationExemptInput.checked = employee.stationAssignmentExempt === true;
...
function saveEmployee() {
  ...
  const taskExemptions = isAssistRow
    ? []
    : employeeTaskExemptionOptions.filter((option) => option.checked).map((option) => option.value);
  const stationAssignmentExempt = isAssistRow ? true : employeeStationExemptInput.checked;
  const fixedDayOff = isAssistRow ? "Monday" : employeeFixedDayOffInput.value;
  const saturdayGroup = isAssistRow ? "1" : employeeSaturdayGroupInput.value || "0";
  const employmentType = isAssistRow ? "assist" : employeeEmploymentTypeInput.value;
  const shiftPoolType = isAssistRow ? "auto" : employeeShiftPoolTypeInput.value || "auto";
  ...
  const employeeRecord = {
    ...
    : existingEmployee?.saturdayRule && String(existingEmployee.saturdayGroup) === String(saturdayGroup)
  ...
  render();
  if (!isAssistRow && getAllBranchShiftPresets().length === 0) {
    showToast("Employee added with starter shifts. Add Branch Rules to save this branch's real shift times.");
  }
  resetEmployeeForm();
  if (shouldReturnToBranchRules) {
    returnToBranchRulesFromEmployees();
...
function generateDefaultWeekPlan(employee, weekType = state.currentWeekType, targetHoursOverride = null) {
  if (employee.assistRow) {
  ...
  const targetHours =
    targetHoursOverride === null
      ? Number(employee.targetHours ?? fallbackTargetHours) || fallbackTargetHours
      : Math.max(0, Number(targetHoursOverride) || 0);
...
function scheduleEmployeesTo40Hours() {
  employees.forEach((employee) => {
    ...
    const employeeUid = getEmployeeUid(employee);
    const sundayHours = getAssignmentHours(ensureAssignment(employeeUid, "Sunday"), employee);
    const remainingTargetHours = Math.max(0, getEmployeeTargetHours(employee) - sundayHours);
    const defaultWeekPlan = generateDefaultWeekPlan(employee, state.currentWeekType, remainingTargetHours);
...
function autoFillCoverageTasks() {
  ...
  const unresolvedCoverage = repairAutoFilledCoverage(finalTaskCounts);
  const actualTaskCounts = getAutoCoverageTaskTotals();

  setDirtyAndAutosave("Coverage auto-filled.");
  render();
  const baseMessage =
    `Auto-filled ${actualTaskCounts.openerAssigned} openers, ${actualTaskCounts.closerAssigned} closers, ` +
    `${actualTaskCounts.nightDropAssigned} night drops, and ${actualTaskCounts.driveThruAssigned} drive thru assignments.`;
  showToast(
    unresolvedCoverage.length
      ? `${baseMessage} Review ${unresolvedCoverage.length} coverage warning${unresolvedCoverage.length === 1 ? "" : "s"}.`
      : baseMessage
  );
}
...
function clearAutoCoverageTasksForDay(day) {
  employees.forEach((employee) => {
    const assignment = ensureAssignment(getEmployeeUid(employee), day);
    assignment.tasks.opener = false;
    assignment.tasks.closingLead = false;
...
function getCoverageTotalsForDay(day) {
...
function repairAutoFilledCoverage(taskCounts) {
  const unresolved = [];

  autoFillWeekDays.forEach((day) => {
    if (isBranchClosedDay(day) || isFullHolidayDay(day)) {
      return;
    }

    // Give each day a few passes because changing a shift to satisfy opening
    // or closing can create a new eligible task candidate.
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const warnings = getDailyCoverageWarnings(day, getCoverageTotalsForDay(day));
      ...
      ensureCoverageShiftsForDay(day);
      limitNineAmStartsForDay(day, { coverageLocked: true });
      clearAutoCoverageTasksForDay(day);
      assignAutoCoverageTasksForDay(day, taskCounts);
    }

    const remainingWarnings = getDailyCoverageWarnings(day, getCoverageTotalsForDay(day));
    if (remainingWarnings.length) {
      unresolved.push({ day, warnings: remainingWarnings });
    }
  });
...
function getAutoCoverageTaskTotals() {
  return employees.reduce(
    (totals, employee) => {
      autoFillWeekDays.forEach((day) => {
        const assignment = ensureAssignment(getEmployeeUid(employee), day);
        if (assignment.tasks.opener) {
          totals.openerAssigned += 1;
        }
        if (assignment.tasks.closingLead) {
          totals.closerAssigned += 1;
        }
        if (assignment.tasks.nightDrop) {
          totals.nightDropAssigned += 1;
        }
        if (assignment.tasks.driveThru) {
          totals.driveThruAssigned += 1;
        }
      });
      return totals;
    },
...
function assignAutoCoverageTasksForDay(day, taskCounts) {
  if (state.autoFillSettings.opener) {
    ensureFinalOpenerCoverage(day);
  }
  if (state.autoFillSettings.closingLead) {
    ensureFinalCloserCoverage(day);
  }
  ...
  const openerCandidate = state.autoFillSettings.opener ? [...eligibleEmployees]
    .filter(({ employee }) => !isEmployeeExemptFromTask(employee, "opener"))
    .sort((left, right) => {
  ...
  const closerPool = state.autoFillSettings.closingLead ? [...eligibleEmployees]
    .filter(({ employee }) => !openerCandidate || getEmployeeUid(employee) !== getEmployeeUid(openerCandidate.employee))
    .filter(({ employee }) => !isEmployeeExemptFromTask(employee, "closingLead"))
    .sort((left, right) => {
      const leftUid = getEmployeeUid(left.employee);
      const rightUid = getEmployeeUid(right.employee);
    ...
    .filter(({ employee, assignment }) => {
      ...
      return coworkersClosing.length >= Math.max(0, getMinimumClosingStaff() - 1);
    }) : [];

  const closerCandidate = closerPool.at(0) || (
    state.autoFillSettings.closingLead
      ? eligibleEmployees
          .filter(({ employee }) => !isEmployeeExemptFromTask(employee, "closingLead"))
          .find(({ assignment }) => {
            const end = getShiftEnd(assignment.shift);
  ...
  const nightDropCandidates = state.autoFillSettings.nightDrop ? [...scheduledEmployees]
    .filter(({ employee }) => !employee.assistRow && !isManagerOrSupervisor(employee))
    .filter(({ employee }) => !isEmployeeExemptFromTask(employee, "nightDrop"))
    .sort((left, right) => {
      const leftUid = getEmployeeUid(left.employee);
      const rightUid = getEmployeeUid(right.employee);
  ...
  const driveThruCandidate = state.autoFillSettings.driveThru ? [...scheduledEmployees]
    .filter(({ employee }) => !employee.assistRow && !isManagerOrSupervisor(employee))
    .filter(({ employee }) => !isEmployeeExemptFromTask(employee, "driveThru"))
    .sort((left, right) => {
      const leftUid = getEmployeeUid(left.employee);
      const rightUid = getEmployeeUid(right.employee);
...
function ensureFinalOpenerCoverage(day) {
  ...
  let eligibleEmployees = scheduledEmployees.filter(
    ({ employee }) => isLeadEligible(employee) && !isEmployeeExemptFromTask(employee, "opener")
  );
...
function ensureFinalCloserCoverage(day) {
  const closingTime = getCoverageClosingTime(day);
  let scheduledEmployees = getScheduledEmployeesForDay(day);

  const leadAlreadyClosing = scheduledEmployees.some(
    ({ employee, assignment }) =>
      isLeadEligible(employee) &&
      !isEmployeeExemptFromTask(employee, "closingLead") &&
      (getShiftEnd(assignment.shift) ?? 0) >= closingTime
  );

  if (!leadAlreadyClosing) {
    const leadCandidate = chooseCoverageCandidate(
    ...
    if (leadCandidate) {
      setAssignmentToCoverageShift(leadCandidate.employee, day, leadCandidate.assignment, "close", { forceCoverage: true });
    }
  }

  scheduledEmployees = getScheduledEmployeesForDay(day);
  let closingStaff = scheduledEmployees.filter(({ assignment }) => (getShiftEnd(assignment.shift) ?? 0) >= closingTime);
  const minimumClosingStaff = getMinimumClosingStaff();
  if (closingStaff.length >= minimumClosingStaff) {
    return;
  }

  const neededClosers = minimumClosingStaff - closingStaff.length;
  scheduledEmployees
    .filter(({ employee, assignment }) => {
        return (
          !employee.assistRow &&
    ...
    .forEach(({ employee, assignment }) => {
      setAssignmentToCoverageShift(employee, day, assignment, "close", { forceCoverage: true });
    });
}

function getScheduledEmployeesForDay(day) {
  // Sunday is normally marked closed, but manual Sunday event shifts should
  // still be visible and count as scheduled hours.
  if (day !== "Sunday" && isBranchClosedDay(day)) {
    return [];
  }
...
function ensureCoverageShiftsForDay(day) {
  ...
  const eligibleLeads = getScheduledEmployeesForDay(day).filter(
    ({ employee }) => isLeadEligible(employee) && !isEmployeeExemptFromTask(employee, "opener")
  );
...
function rebalanceHoursAfterCoverage() {
  employees.forEach((employee) => {
    ...
    const employeeUid = getEmployeeUid(employee);
    const targetHours = getEmployeeTargetHours(employee);
    const adjustableDays = autoFillWeekDays.filter((day) => isDayAdjustableForHours(employee, day));
...
function rebalanceHoursWithCoverageLocked() {
  employees.forEach((employee) => {
    ...
    const employeeUid = getEmployeeUid(employee);
    const targetHours = getEmployeeTargetHours(employee);
    const adjustableDays = autoFillWeekDays.filter((day) => isDayAdjustableWithCoverageLocked(employee, day));
...
function getCoverageLockedRebalanceOptions(employee, day) {
  ...
  getAvailableWeekdayShifts(employee).forEach((shift) => {
    ...
    if (!assignment.tasks.closingLead && supportsCloseNow && closingStaffCount <= minimumClosingStaff && !supportsCloseNext) {
      return;
    }
  ...
  if (day !== employee.fixedDayOff) {
    ...
    if (
      ...
      !(supportsCloseNow && closingStaffCount <= minimumClosingStaff)
    ) {
      options.add("OFF");
    }
  }
...
function getCoverageOpeningTime(day) {
  const branchHours = normalizeBranchHours(state.branchHours);
  const defaultOpeningTime = parseShiftTime(day === "Saturday" ? branchHours.saturdayOpen : branchHours.weekdayOpen);
...
function shuffleArray(items, seed = "") {
  const result = [...items];
  let stateValue = Array.from(seed).reduce((sum, char) => sum + char.charCodeAt(0), result.length || 1);
  for (let index = result.length - 1; index > 0; index -= 1) {
    stateValue = (stateValue * 9301 + 49297) % 233280;
...
function getCoverageClosingTime(day) {
  const branchHours = normalizeBranchHours(state.branchHours);
  return parseShiftTime(day === "Saturday" ? branchHours.saturdayClose : branchHours.weekdayClose);
}

function isLeadEligible(employee) {
  return employee?.assistRow !== true && employee?.leadEligible === true;
}

function isManagerOrSupervisor(employee) {
  const text = [employee?.name, employee?.keyType, ...(Array.isArray(employee?.roles) ? employee.roles : [])]
    .join(" ")
    .toLowerCase();
  return text.includes("manager") || text.includes("supervisor") || /\bsup\b/.test(text);
}

function isEmployeeEligibleForStationAssignment(employee, settings = state.stationAutoFillSettings) {
  if (!employee || employee.assistRow || employee.stationAssignmentExempt === true) {
    return false;
  }

  const normalizedSettings = normalizeStationAutoFillSettings(settings);
  if (normalizedSettings.excludeManagers && isManagerOrSupervisor(employee)) {
    return false;
  }
...
function isEmployeeExemptFromTask(employee, taskId) {
  return Array.isArray(employee?.taskExemptions) && employee.taskExemptions.includes(taskId);
}

function getTaskExemptionSummary(employee) {
  if (employee.assistRow) {
    return "";
  }

  const labels = Array.isArray(employee.taskExemptions) ? employee.taskExemptions
    .map((taskId) => builtInTaskConfig[taskId]?.label)
    .filter(Boolean) : [];
  if (employee.stationAssignmentExempt) {
    labels.push("Station / Office");
  }
  return labels.length ? `Auto-fill skips: ${labels.join(", ")}` : "";
}
...
function populateShiftEditorOptions(currentShift, day, employee = null) {
  ...
  const editorPresets = [...getAllBranchShiftPresets(), ...shiftPresets];
  ...
  const optionGroups = [
    {
      label: "Status",
      options: [closedCellShift, "OFF", "PTO", holidayShift, darkCellShift, saturdayShift],
    },
    {
      label: "Branch Times",
      options: getAllBranchShiftPresets(),
    },
  ];
  ...
  if (currentShift && !editorPresets.includes(currentShift)) {
...
function clearWorkspaceToBlank() {
  ...
  state.assignments = {};
  state.customTasks = [];
  state.branchShiftPresets = [];
  state.branchPartTimeShiftPresets = [];
  state.customShiftPresets = [];
  state.rotationWeekCount = defaultRotationWeekCount;
  state.rotationWeekCountConfigured = false;
  state.currentWeekType = "Custom";
  state.cellEditorExpanded = false;
  state.autoFillSettings = normalizeAutoFillSettings();
  state.branchHours = normalizeBranchHours();
  state.branchClosedDays = ["Sunday"];
  state.stationAutoFillSettings = normalizeStationAutoFillSettings();
  state.loadedSaveId = "";
  state.loadedSaveName = getAutosaveDraftName();
  state.lastAutosavedAt = "";
...
function applyQuickCellShortcut(cellId, shortcutType) {
  ...
  if (shortcutType === "OFF") {
    if (["OFF", closedCellShift].includes(assignment.shift)) {
...
function getDefaultSaturdayRuleForEmployee(employee) {
  const activeWeeks = getActiveWeekTypeOptions();
  if ((employee.saturdayGroup === undefined || employee.saturdayGroup === null || employee.saturdayGroup === "") && employee.saturdayRule?.workWeeks?.length) {
    return {
      workWeeks: employee.saturdayRule.workWeeks.filter((week) => activeWeeks.includes(week)),
      label: employee.saturdayRule.label || "",
    };
  }

  const rawSaturdayGroup = String(employee.saturdayGroup ?? "1");
  if (rawSaturdayGroup === "odd") {
    const offWeeks = activeWeeks.filter((_, index) => index % 2 === 0);
    return createSaturdayRuleForOffWeeks(offWeeks, `${offWeeks.map(getWeekLetter).join("/")} Off`);
  }
  if (rawSaturdayGroup === "even") {
    const offWeeks = activeWeeks.filter((_, index) => index % 2 === 1);
    return createSaturdayRuleForOffWeeks(offWeeks, `${offWeeks.map(getWeekLetter).join("/")} Off`);
  }

  const allowedGroups = Array.from({ length: activeWeeks.length }, (_, index) => index + 1);
  const saturdayGroup = [0, ...allowedGroups].includes(Number(rawSaturdayGroup))
    ? Number(rawSaturdayGroup)
    : 1;
  if (saturdayGroup === 0) {
    return createSaturdayRule(activeWeeks, "Works every rotating Saturday");
  }
...
function getAvailableWeekdayShifts(employee) {
  const shiftPool = getSchedulingShiftPool(employee);
  if (getEmployeeShiftPoolType(employee) === "part_time") {
    return shiftPool.filter((shift) => getShiftHours(shift, employee) <= 5 && isShiftAllowedForEmployee(employee, "Monday", shift));
  }

  const shifts = [...shiftPool];
...
function getSchedulingShiftPool(employee) {
  const branchShifts = getBranchShiftPresets(getEmployeeShiftPoolType(employee));
  if (branchShifts.length > 0) {
    return branchShifts;
  }

  return getEmployeeShiftPoolType(employee) === "part_time"
    ? partTimeShiftOptions
    : starterScheduleShiftPresets;
}

function getEmployeeShiftPoolType(employee) {
  if (employee?.assistRow) {
    return "full_time";
  }
  if (["full_time", "part_time"].includes(employee?.shiftPoolType)) {
    return employee.shiftPoolType;
  }
  return employee?.employmentType === "part_time" ? "part_time" : "full_time";
}
...
function ensureAssignment(employeeUid, day) {
  ...
  const fallbackShift = day === "Sunday" ? closedCellShift : "OFF";
  state.assignments[employeeUid][day] = normalizeAssignment(state.assignments[employeeUid][day], fallbackShift);
...
async function exportSchedule(data, options = {}) {
  ...
  const savedFilename = await saveJsonFile(JSON.stringify(exportPayload, null, 2), filename);
  if (!savedFilename) {
    showToast("Save canceled.");
    return;
  }

  markExportCompleted(savedFilename, options);
}

async function saveJsonFile(text, filename) {
  ...
  if (window.showSaveFilePicker) {
    try {
      ...
      return fileHandle.name || safeFilename;
    } catch (error) {
  ...
  const fallbackName = window.prompt("Name this JSON file:", safeFilename);
  if (fallbackName === null) {
    return "";
  }
  const finalFilename = ensureJsonFilename(fallbackName.trim() || safeFilename);
  downloadTextFile(text, finalFilename);
  return finalFilename;
}

function ensureJsonFilename(filename) {
  const cleaned = String(filename || "schedule.json")
    .trim()
    .replace(/[\\/:*?"<>|]+/g, "-");
...
function normalizeLoadedSchedule(raw) {
  ...
  const normalizedRotationWeekCountConfigured =
    payload.rotationWeekCountConfigured === true ||
    (sourceVersion >= 7 && payload.rotationWeekCount !== undefined && payload.rotationWeekCount !== "");
  const normalizedRotationWeekCount = normalizedRotationWeekCountConfigured
    ? normalizeRotationWeekCount(payload.rotationWeekCount)
    : sourceVersion >= 6 && payload.rotationWeekCount !== undefined
      ? ""
      : inferRotationWeekCount(payload, normalizedEmployees);
  const activeWeekTypes = weekTypeOptions.slice(0, normalizedRotationWeekCount);
  const normalizedWeekType = [...activeWeekTypes, "Custom"].includes(payload.currentWeekType)
    ? payload.currentWeekType
    : "Week A";

  state.rotationWeekCount = normalizedRotationWeekCount;
  if (sourceVersion < 3) {
  ...
  normalizedEmployees.forEach((employee) => {
    ...
    weekDays.forEach((day) => {
      ...
      const rawAssignment = rawAssignments[employeeUid]?.[day];
      const normalizedAssignment = normalizeAssignment(
        rawAssignment,
        defaultWeekPlan[day] ?? "OFF"
      );
      // Version 10 introduced Sunday as a manual-only column with OFF as its
      // temporary placeholder. Migrate that placeholder to the clearer
      // CLOSED status while preserving any real Sunday shift.
      if (day === "Sunday" && sourceVersion < 11 && (!rawAssignment || rawAssignment.shift === "OFF")) {
        normalizedAssignment.shift = closedCellShift;
      }
      normalizedAssignments[employeeUid][day] = normalizedAssignment;
    });
  });
  ...
  return {
    ...
    payload: {
      ...
      rotationWeekCount: normalizedRotationWeekCount,
      rotationWeekCountConfigured: normalizedRotationWeekCountConfigured,
      cellEditorDock: payload.cellEditorDock === "left" ? "left" : "right",
      cellEditorExpanded: payload.cellEditorExpanded === true,
      stationScheduleEnabled: payload.stationScheduleEnabled !== false,
      autoFillSettings: normalizeAutoFillSettings(payload.autoFillSettings),
      branchHours: normalizeBranchHours(payload.branchHours),
      branchClosedDays: normalizeBranchClosedDays(
        sourceVersion < 11
          ? [...(Array.isArray(payload.branchClosedDays) ? payload.branchClosedDays : []), "Sunday"]
          : payload.branchClosedDays
      ),
      stationAutoFillSettings: normalizeStationAutoFillSettings(payload.stationAutoFillSettings),
      ...
      branchShiftPresets: normalizeBranchShiftPresets(payload.branchShiftPresets, payload.customShiftPresets),
      branchPartTimeShiftPresets: normalizeBranchShiftPresets(payload.branchPartTimeShiftPresets, []),
      customShiftPresets: normalizeCustomShiftPresets(payload.customShiftPresets),
    },
  };
}
...
function applyLoadedData(normalized, options = {}) {
  ...
  state.rotationWeekCount = normalizeRotationWeekCount(payload.rotationWeekCount);
  state.rotationWeekCountConfigured = payload.rotationWeekCountConfigured === true;
  ...
  state.cellEditorExpanded = payload.cellEditorExpanded === true;
  state.stationScheduleEnabled = payload.stationScheduleEnabled !== false;
  state.autoFillSettings = normalizeAutoFillSettings(payload.autoFillSettings);
  state.branchHours = normalizeBranchHours(payload.branchHours);
  state.branchClosedDays = normalizeBranchClosedDays(payload.branchClosedDays);
  state.stationAutoFillSettings = normalizeStationAutoFillSettings(payload.stationAutoFillSettings);
  state.weekStart = getMonday(new Date(`${payload.weekStart}T12:00:00`));
  state.assignments = payload.assignments || {};
  applyClosedDayRules();
  state.customTasks = payload.customTasks || [];
  state.branchShiftPresets = normalizeBranchShiftPresets(payload.branchShiftPresets, payload.customShiftPresets);
  state.branchPartTimeShiftPresets = normalizeBranchShiftPresets(payload.branchPartTimeShiftPresets, []);
  state.customShiftPresets = normalizeCustomShiftPresets(payload.customShiftPresets);
  weekDays.forEach((day) => {
...
function getSerializableState() {
  return {
    ...
    rotationWeekCount: normalizeRotationWeekCount(state.rotationWeekCount),
    rotationWeekCountConfigured: state.rotationWeekCountConfigured === true,
    cellEditorDock: state.cellEditorDock,
    cellEditorExpanded: state.cellEditorExpanded,
    stationScheduleEnabled: state.stationScheduleEnabled,
    autoFillSettings: cloneData(state.autoFillSettings),
    branchHours: cloneData(normalizeBranchHours(state.branchHours)),
    branchClosedDays: cloneData(normalizeBranchClosedDays(state.branchClosedDays)),
    stationAutoFillSettings: cloneData(state.stationAutoFillSettings),
    ...
    branchShiftPresets: cloneData(getBranchShiftPresets()),
    branchPartTimeShiftPresets: cloneData(getBranchShiftPresets("part_time")),
    customShiftPresets: cloneData(state.customShiftPresets),
  };
}
...
function saveCustomShiftPreset(shift) {
  const normalized = normalizeShiftRangeInput(shift);
  if (!normalized || getBranchShiftPresets().includes(normalized)) {
    return;
  }

  state.branchShiftPresets = [...getBranchShiftPresets(), normalized];
  state.customShiftPresets = [];
  setDirtyAndAutosave("Custom shift saved to this branch.");
}
...
function hasVisibleModal() {
  return !confirmModal.classList.contains("is-hidden") ||
    ...
    !autoFillOptionsModal.classList.contains("is-hidden") ||
    !stationAutoFillModal.classList.contains("is-hidden") ||
    !branchRulesModal.classList.contains("is-hidden") ||
    !firstTimeSetupModal.classList.contains("is-hidden");
}
...
function askForCustomShift(currentShift = "") {
  ...
  customShiftSavePresetInput.checked = false;
  ...
  return new Promise((resolve) => {
    ...
    const onAccept = () => {
      ...
      if (customShiftSavePresetInput.checked) {
        saveCustomShiftPreset(shift);
      }
      cleanup(shift);
    };
    const onBackdrop = (event) => {
...
function getShiftOptionLabel(preset) {
  ...
  if (preset === closedCellShift) {
    return "Closed (Sunday default)";
  }
...
function normalizeAutoFillSettings(settings = {}) {
  return {
    opener: settings.opener !== false,
    closingLead: settings.closingLead !== false,
    nightDrop: settings.nightDrop !== false,
    driveThru: settings.driveThru !== false,
    minimumClosingStaff: normalizeMinimumClosingStaff(settings.minimumClosingStaff),
  };
}

function normalizeMinimumClosingStaff(value) {
...
function normalizeBranchHours(hours = {}) {
  const defaults = {
    weekdayOpen: "8:00",
    weekdayClose: "6:00",
    saturdayOpen: "8:30",
  ...
  const normalized = Object.fromEntries(
    Object.entries(defaults).map(([key, fallback]) => {
      const value = normalizeShiftTimeLabel(hours?.[key] || fallback);
      return [key, value || fallback];
    })
  );

  if (!isBranchHoursRangeValid(normalized.weekdayOpen, normalized.weekdayClose)) {
    normalized.weekdayOpen = defaults.weekdayOpen;
    normalized.weekdayClose = defaults.weekdayClose;
  }
  if (!isBranchHoursRangeValid(normalized.saturdayOpen, normalized.saturdayClose)) {
    normalized.saturdayOpen = defaults.saturdayOpen;
    normalized.saturdayClose = defaults.saturdayClose;
  }

  return normalized;
}

function normalizeBranchClosedDays(days = ["Sunday"]) {
...
function isBranchClosedDay(day) {
  return normalizeBranchClosedDays(state.branchClosedDays).includes(day);
}

function applyClosedDayRules() {
  // Do not clear Sunday. It is a manual-only event day even when the branch
  // is marked closed in Branch Rules.
  const closedDays = normalizeBranchClosedDays(state.branchClosedDays).filter((day) => day !== "Sunday");
  ...
  employees.forEach((employee) => {
    ...
    closedDays.forEach((day) => {
      ...
      assignment.lobbyLead = "";
      assignment.ptoHours = 0;
      if (assignment.cellColor === darkCellColor) {
        assignment.cellColor = "";
      }
    });
  });
...
function isBranchHoursRangeValid(openTime, closeTime) {
  const openMinutes = parseShiftTime(openTime);
  const closeMinutes = parseShiftTime(closeTime);
  return openMinutes !== null && closeMinutes !== null && closeMinutes > openMinutes;
}

function normalizeStationAutoFillSettings(settings = {}) {
  const stationCount = Math.max(0, Number(settings.stations) || 0);
  const officeCount = Math.max(0, Number(settings.offices) || 0);
  const legacyLocations = [
    ...
    ...Array.from({ length: officeCount }, (_, index) => ({
      type: "office",
      label: `Office ${index + 1}`,
      lunch: "",
    })),
  ];
  const rawLocations = Array.isArray(settings.locations) ? settings.locations : legacyLocations;
  const locations = rawLocations
    .map((location, index) => ({
      id: location.id || `location-${index + 1}-${Math.random().toString(36).slice(2, 6)}`,
      type: location.type === "office" ? "office" : "station",
      label: String(location.label || `${location.type === "office" ? "Office" : "Station"} ${index + 1}`).trim(),
      lunch: normalizeShiftTimeLabel(location.lunch || "") || "",
    }))
    .filter((location) => location.label);
  ...
  return {
    ...
    offices: officeCount,
    locations,
    randomize: settings.randomize !== false,
    lobbyLeadMinutes: Math.max(30, Number(settings.lobbyLeadMinutes) || 60),
    excludeManagers: settings.excludeManagers !== false,
  };
}

function normalizeCustomShiftPresets(presets = []) {
  if (!Array.isArray(presets)) {
    return [];
  }

  return [...new Set(presets.map((shift) => normalizeShiftRangeInput(shift)).filter(Boolean))];
}

function normalizeBranchShiftPresets(presets = [], legacyCustomPresets = []) {
  const source = Array.isArray(presets) ? presets : defaultBranchShiftPresets;
  return [
    ...new Set(
      [...source, ...(Array.isArray(legacyCustomPresets) ? legacyCustomPresets : [])]
        .map((shift) => normalizeShiftRangeInput(shift))
        .filter(Boolean)
    ),
  ];
}
...
function getFederalHolidaysForYear(year) {
  const fixedHolidays = [
    ...
    name: holiday.name,
    // Show the holiday on its actual calendar date. Branches can still choose
    // the observed paid day manually with the Holiday cell option if needed.
    date: formatDateInput(new Date(year, holiday.month, holiday.day)),
  }));
...
function hydrateEmployees(employeeList) {
  employeeList.forEach((employee, index) => {
    ...
    const saturdayGroup = String(employee.saturdayGroup ?? "1");
    const activeSaturdayGroups = Array.from({ length: getActiveWeekTypeOptions().length }, (_, index) => index + 1);
    employee.saturdayGroup =
      ["odd", "even"].includes(saturdayGroup) || [0, ...activeSaturdayGroups].includes(Number(saturdayGroup))
        ? saturdayGroup
        : "1";
    ...
    employee.shiftPoolType = ["full_time", "part_time"].includes(employee.shiftPoolType)
      ? employee.shiftPoolType
      : "auto";
    ...
    employee.taskExemptions = employee.assistRow
      ? []
      : Array.isArray(employee.taskExemptions)
        ? employee.taskExemptions.filter((taskId) => ["opener", "closingLead", "nightDrop", "driveThru"].includes(taskId))
        : [];
    employee.stationAssignmentExempt = employee.assistRow ? true : employee.stationAssignmentExempt === true;
  });
}
...
function syncEmployeeRowTypeFields() {
  ...
  employeeShiftPoolTypeInput.disabled = isAssistRow;
  employeeStationExemptInput.disabled = isAssistRow;
  ...
  if (isAssistRow) {
    ...
    employeeStationExemptInput.checked = true;
    employeeLeadEligibleInput.checked = false;
    employeeCustomRolesInput.value = "";
    employeeRoleOptions.forEach((option) => {
      option.checked = false;
...
function downloadScheduleHtml() {
  ...
  const styles = `
    ...
    .note-pill.custom-task-purple, .note-purple { background: #e7ddff; color: #1f1740; border-color: #b9a7f5; }
    .note-pill.custom-task-hotpink, .note-hotpink { background: #ff5cad; color: #111; border-color: #ff5cad; }
    .note-pill.custom-task-teal, .note-teal { background: #a8f1ed; color: #093c3c; border-color: #58c7c2; }
