...
function requireBranchShiftRulesForAutoFill() {
  if (getBranchShiftPresets().length > 0) {
    return true;
  }
...
function renderBranchShiftRules() {
  branchShiftList.innerHTML = "";
  const shifts = getBranchShiftPresets();
  if (shifts.length === 0) {
    branchShiftStatus.textContent = "No branch shift times saved yet. Add the clock-in/out times this branch actually uses.";
    branchShiftList.appendChild(buildEmptyStateRow("No shift times yet. Add times like 8-5:45 or 10-2 to begin."));
    return;
  }

  branchShiftStatus.textContent = `${shifts.length} shift times saved for this branch.`;
  shifts.forEach((shift) => {
    branchShiftList.appendChild(buildBranchShiftRow(shift));
  });
}
...
function buildBranchShiftRow(shift) {
  ...
  input.addEventListener("blur", () => {
    ...
    if (!normalized) {
      ...
      branchShiftStatus.textContent = "Shift time restored. Use a valid time like 8-5:45.";
      return;
    }
  ...
  removeButton.addEventListener("click", () => {
    row.remove();
    if (branchShiftList.querySelectorAll(".branch-rule-row").length === 0) {
      branchShiftList.appendChild(buildEmptyStateRow("No shift times yet. Add times like 8-5:45 or 10-2 to begin."));
    }
  ...
  row.addEventListener("dragend", () => {
    ...
    branchShiftList.querySelectorAll(".branch-rule-row").forEach((entry) => {
  ...
  row.addEventListener("dragover", (event) => {
    event.preventDefault();
    const draggingRow = branchShiftList.querySelector(".branch-rule-row.is-dragging");
    if (!draggingRow || draggingRow === row) {
      return;
    }

    const insertAfter = event.clientY > row.getBoundingClientRect().top + row.offsetHeight / 2;
    branchShiftList.insertBefore(draggingRow, insertAfter ? row.nextSibling : row);
...
function addBranchShiftFromInput() {
  const normalized = normalizeShiftRangeInput(branchShiftInput.value);
  if (!normalized) {
    branchShiftStatus.textContent = "Enter a valid shift like 8-5:45 or 8:00-1:30.";
    return;
  }

  const current = getBranchShiftRulesFromModal();
  if (current.includes(normalized)) {
    branchShiftStatus.textContent = `${normalized} is already saved.`;
    branchShiftInput.value = "";
    return;
  }

  branchShiftList.querySelector(".setup-empty-state")?.remove();
  branchShiftList.appendChild(buildBranchShiftRow(normalized));
  branchShiftInput.value = "";
  applyBranchRulesAutomatically(`${normalized} added.`);
}

function getBranchShiftRulesFromModal() {
  return normalizeBranchShiftPresets(
    Array.from(branchShiftList.querySelectorAll(".branch-rule-row input"))
...
function getBranchShiftPresets() {
...
function saveBranchRules({ silent = false } = {}) {
  ...
  state.branchShiftPresets = getBranchShiftRulesFromModal();
...
function autoFillStationPlan() {
  ...
  weekDays.forEach((day) => {
    const scheduledRows = getScheduledEmployeesForDay(day)
    ...
    scheduledRows.forEach(({ employee, assignment }, index) => {
      const location = locationPool.length ? locationPool[index % locationPool.length] : null;
      ...
      if (start !== null && end !== null) {
        const lobbyStart = Math.max(start, getCoverageOpeningTime(day)) + (index % 4) * settings.lobbyLeadMinutes;
        const lobbyEnd = Math.min(lobbyStart + settings.lobbyLeadMinutes, end);
        assignment.lobbyLead = lobbyEnd > lobbyStart ? `${formatScheduleTime(lobbyStart)}-${formatScheduleTime(lobbyEnd)}` : "";
      }
...
function saveEmployee() {
  ...
  render();
  if (!isAssistRow && getBranchShiftPresets().length === 0) {
...
function assignAutoCoverageTasksForDay(day, taskCounts) {
  ...
  const closerCandidate = state.autoFillSettings.closingLead ? [...eligibleEmployees]
...
function getTaskExemptionSummary(employee) {
  if (employee.assistRow || !Array.isArray(employee.taskExemptions) || employee.taskExemptions.length === 0) {
    return "";
  }

  const labels = employee.taskExemptions
...
function populateShiftEditorOptions(currentShift, day, employee = null) {
  ...
  const editorPresets = [...getBranchShiftPresets(), ...shiftPresets];
...
function getSchedulingShiftPool(employee) {
  const branchShifts = getBranchShiftPresets();
